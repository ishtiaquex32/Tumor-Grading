...
val_loader = DataLoader(
    UCSFX3DDataset(val_df, IMAGE_FOLDER),
    batch_size=BATCH_SIZE,
    shuffle=False,
    num_workers=4,
    pin_memory=torch.cuda.is_available(),
...
test_loader = DataLoader(
    UCSFX3DDataset(test_df, IMAGE_FOLDER),
    batch_size=BATCH_SIZE,
    shuffle=False,
    num_workers=4,
    pin_memory=torch.cuda.is_available(),
...
class X3DMRIClassifier(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.x3d = torch.hub.load('facebookresearch/pytorchvideo', 'x3d_m', pretrained=True)
        print(self.x3d)

        in_features = self.x3d.blocks[5].proj.in_features

        self.x3d.blocks[5].pool = nn.AdaptiveAvgPool3d(1)

        self.x3d.blocks[5].proj = nn.Linear(192, 2)
